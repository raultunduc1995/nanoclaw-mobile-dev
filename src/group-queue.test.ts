...
describe('GroupQueue', () => {
  ...
  it('only runs one container per group at a time', async () => {
    ...
    queue.enqueueMessageCheck('tg:group_one');
    queue.enqueueMessageCheck('tg:group_one');
  ...
  it('respects global concurrency limit', async () => {
    ...
    queue.enqueueMessageCheck('tg:group_one');
    queue.enqueueMessageCheck('tg:group_two');
    queue.enqueueMessageCheck('tg:group_three');
  ...
  it('drains tasks before messages for same group', async () => {
    ...
    queue.enqueueTask('tg:group_one', 'task-1', taskFn);
    queue.enqueueMessageCheck('tg:group_one');
  ...
  it('drains waiting groups when active slots free up', async () => {
    ...
    expect(processed).toEqual(['tg:group_one', 'tg:group_two']);

    // Free up a slot
    completionCallbacks[0]();
    await vi.advanceTimersByTimeAsync(10);

    expect(processed).toContain('tg:group_three');
  });
  ...
  it('rejects duplicate enqueue of a currently-running task', async () => {
    ...
    queue.enqueueTask('tg:group_one', 'task-1', dupFn);
    await vi.advanceTimersByTimeAsync(10);
  ...
  it('does NOT preempt active container when not idle', async () => {
    ...
    queue.registerProcess('tg:group_one', {} as any, 'container-1', 'test-group');
  ...
  it('sendMessage resets idleWaiting so a subsequent task enqueue does not preempt', async () => {
    ...
    queue.sendMessage('tg:group_one', 'hello');
  ...
  it('sendMessage returns false for task containers so user messages queue up', async () => {
    ...
    const result = queue.sendMessage('tg:group_one', 'hello');
