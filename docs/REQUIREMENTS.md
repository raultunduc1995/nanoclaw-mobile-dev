...
When people contribute, they should contribute a skill like `/add-telegram` that transforms the codebase. Some features (like WhatsApp) are bundled and gated by feature flags. Others are installed via branch merges. Users fork the repo, run skills to customize, and end up with clean code that does exactly what they need - not a bloated system trying to support everyone's use case simultaneously.
...
- `/add-telegram` - Add Telegram as an input channel (exists)
- `/add-slack` - Add Slack as an input channel (exists)
- `/add-discord` - Add Discord as an input channel (exists)
- `/add-gmail` - Add Gmail as a channel (exists)
- `/add-sms` - Add SMS via Twilio or similar

### Container Runtime
The project uses Docker by default (cross-platform). For macOS users who prefer Apple Container:
- `/convert-to-apple-container` - Switch from Docker to Apple Container (macOS-only) (exists)
...
- **Multi-channel support** (WhatsApp, Telegram, Slack, Discord, Gmail) via self-registering channel modules
...
### WhatsApp
- Bundled feature module (`src/whatsapp/`), gated by `ENABLE_WHATSAPP_CHANNEL` feature flag
- Using baileys library for WhatsApp Web connection
- Messages stored in SQLite, polled by router
- QR code or pairing code authentication during setup
