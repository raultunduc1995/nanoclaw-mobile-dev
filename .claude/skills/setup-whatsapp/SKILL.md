...
## Phase 2: Enable WhatsApp feature flag

Add `ENABLE_WHATSAPP=true` to `.env` (creates the file if it doesn't exist):

```bash
grep -q '^ENABLE_WHATSAPP=' .env 2>/dev/null && sed -i '' 's/^ENABLE_WHATSAPP=.*/ENABLE_WHATSAPP=true/' .env || echo "ENABLE_WHATSAPP=true" >> .env
```

Verify:

```bash
grep ENABLE_WHATSAPP .env
```
...
1. Disable the feature flag: `sed -i '' 's/^ENABLE_WHATSAPP=.*/ENABLE_WHATSAPP=false/' .env`
